...
[ -f /etc/modprobe.d/tuned.conf ] && cp -v /etc/modprobe.d/tuned.conf /etc/modprobe.d/tuned.conf-modprobe	
lsmod >> lsmod
...
[ -f /etc/multipath.conf ] && cp -v /etc/multipath.conf .
...
ip a >> ip-a
VL=$(ifconfig | egrep "Link encap|flags" | grep -v lo| awk '{print $1}' | cut -d':' -f1);echo Interfaces:"$VL"; for i in $VL;do ethtool "$i" >> ethtool-"$i";ip -s link show $i >> link-status-$i; done
...
echo "-----------3.6.3"
grep -i crashkernel /etc/grub2.cfg >> kdump
...
echo "-----------3.7.3"
cp -v /boot/grub2/grub.cfg .
grep -i elevator /etc/grub2.cfg >> io-scheduler

echo "-----------3.7.4"
grep -i nousb /etc/grub2.cfg >> target-security-policy
